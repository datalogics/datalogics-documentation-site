...
- Add PDOCRRecognizePage C++ API to OCR plugin to run optical character recognition (OCR) on a page.
- The previous APDFL release (v18.0.5PlusP1w) introduced OCREngine support in the C++ interface, along with the OCRImage sample that demonstrates importing a bitmap into a PDF and adding OCR text.
- This release builds upon that functionality by adding the PDOCRRecognizePage API, which rasterizes an entire page and performs OCR on the resulting image. A new OCRPage sample is included to demonstrate this workflow.
