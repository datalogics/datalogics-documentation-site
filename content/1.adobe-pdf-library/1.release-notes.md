...
## APDFL v18.0.5PlusP2c (April 17, 2026)

**New Features:**

- Add support for PAdES (B-T) baseline electronic signatures to .NET/Java interfaces. :br This enhancement enables creation and validation of signatures that include trusted timestamps, ensuring long-term integrity and compliance with ETSI PAdES baseline standards.
- Add support for PAdES-EPES policy-based electronic signatures to .NET/Java interfaces. :br This enhancement enables signatures to include an explicit signature policy identifier, ensuring that signing operations adhere to defined organizational or regulatory requirements.

**Changes:**

- Updated zlib from 1.3.1 to 1.3.2 to mitigate CVEs.
- Updated sqlite3 from 3.48.0 to 3.51.0 to mitigate CVE-2025-3277, CVE-2025-29087, CVE-2025-29088, CVE-2025-52099, CVE-2025-6965, CVE-2025-7709.
- Updated libexpat from 2.7.4 to 2.7.5 to mitigate CVEs.
- Updated libpng from 1.6.55 to 1.6.56 to mitigate CVE-2026-33416 and CVE-2026-33636.
- Updated OpenSSL from 3.5.5 to 3.5.6 to mitigate CVE-2026-31790 (RSA KEM) and CVE-2026-31789 (heap buffer overflow).
- With this release, the Maven package version has been increased to 18.62.0.
- With this release, the NuGet package version has been increased to 18.62.0.

**Problem Corrections:**

- SF46861 – Fix for malformed Brotli output, including broken xref streams and predictor warnings, ensuring Brotli-encoded documents open cleanly and display responsively in PDFViewer.
- SF47422 – Rendering of Text using a Type 0 font that's not CJK based has been improved to use a fallback font when possible.
- SF47424 – Improves output for Flattening a XFA Text Field using RichText where the text uses multiple different font size attributes spanning multiple pages using Forms Extension.
- SF47426 – Support for opening PDFs with problematic ObjStm's has been improved.
- SF47434 – Corrects a crash that could occur processing a purposely crafted file with a long string.
- SF47439 – Corrects a problem where Color Conversion of Forms on a page could unexpectedly alter them when shared on a different page.
- SF47448 – Corrects a crash that could occur rendering complicated paths in rare situations.
- SF47460 – Corrects an issue where some Static XFA documents would not be Flattened AsIfPrinted correctly in some cases using Forms Extension.
- SF47461 – Corrects an issue where the Caption of a Text Field shared across pages of an XFA document could end up being placed multiple times on the Flattened document using Forms Extension.
- SF47462 – Corrects an issue where some Static XFA documents would not be Flattened correctly in some cases using Forms Extension.
- SF47463 – Addresses two issues with XFA documents using JavaScript via Forms Extension:
  - Addresses a problem where XFA Text fields using Javascript could have new lines removed in the Flattened output unexpectedly.
  - Corrects a Stack Overflow that could occur flattening some documents using Javascript in rare scenarios.
- SF47466 – Corrects a problem where Flattening a XFA document with a ChoiceList field with a null picture clause could mishandle the data in the Flattened output using Forms Extension.
- SF47467 – Corrects a problem where the WordFinder Quad coordinates of L2R text could be incorrect in some cases.
- SF47468 – Improves output for Flattening a XFA Text Field using RichText where the text uses multiple different spacing attributes spanning multiple pages using Forms Extension.
- SF47479 – Corrects a problem where XFA Static Text elements using a Processing Instruction could be ignored in the Flattened output when using Forms Extension.
- SF47499 – Opening of corrupted PDF documents has been improved to be more resilient.
- SF47506 – Corrects an intermittent crash that could occur when using PDPageDrawContentsToWindowWithParams().
- SF47520 – The minimum macOS version supported is now 12.x (Monterey). Note this version is no longer supported by Apple.

## APDFL v18.0.5PlusP2a (March 13, 2026)

**New Features:**

- API updates to facilitate generation of ETSI PAdES-EPES (policy-based) signatures. :br This allows developers to define and include signature policy identifiers and associated policy information during signature creation, in compliance with ETSI PAdES specifications.

Constraints:

- ETSI PAdES signatures are unavailable on Linux 32-bit and AIX 64-bit platforms.

**Changes:**

- FoxIt Conversion SDK 3.0 for Office Conversion has been updated to 3.0
- Mitigate CVE's by updating libpng 1.6.55
- Mitigate CVE's by updating libcurl to 1.18.0
- APDFL.SharedLibs NuGet package has been increased to 1.0.7
- With this release, the Maven package version has been increased to 18.59.0
- With this release, the NuGet package version has been increased to 18.59.0

**Problem Corrections:**

- SF47324 – Corrects an issue where conversion of some PDF documents to Word would have incorrect orientation.
- SF47427 – Fixes a crash that could occur Rendering a document with a problematic ColorSpace.

## APDFLv18.0.5PlusP1z (February 13, 2026)

**New Features:**

- API updates to facilitate generation of PAdES BES (B-T) signatures. :br Added support for PAdES B-T baseline signatures via a new API implementing the PAdES signature class for PDF documents.

Constraints:

- Not supported on platforms without RFC 3161 timestamp signature support (e.g., linux-x86, aix-64).

**Changes:**

- SF47312 – On Linux, Link-time symbol visibility has been changed to prevent leaking of symbols from 3rd party static archives, which caused problems with [libDL180pdfl.so](http://libDL180pdfl.so) when used with shared versions of some of those libraries.
- Mitigate CVE-2025-15467 by updating OpenSSL to 3.5.5
- Mitigate expat CVE-2026-24515 and CVE-2026-25210 by updating to 2.7.4
- Mitigate CVE's by updating libpng to 1.6.54
- APDFL.SharedLibs NuGet package has been increased to 1.0.6
- With this release, the Maven package version has been increased to [18.58.0](https://wiki-datalogics.atlassian.net/wiki/spaces/PROD/pages/285474817/APDFL+Maven+Package+Releases#Version-18.58.0)
- With this release, the NuGet package version has been increased to [18.58.0](https://wiki-datalogics.atlassian.net/wiki/spaces/PROD/pages/83853379/APDFL+NuGet+Package+Releases#Version-18.58.0)

**Problem Corrections:**

- SF46275 – Corrects an issue in the Flattener would create Outlines for Text even if the useTextOutilnes parameter was not set.
- SF47286 – When a specific OutputIntent is specified for PDF/A conversion it's now utilized as the target colorspace instead of the default (sRGB for RGB or US Web Coated SWOP v2 for CMYK).
- SF46903 – When rendering a PDF with an Image utilizing an invalidly specified ICC Colorspace, processing has been improved in some cases.
  - When optimizing a PDF with an Image utilizing an invalidly specified ICC Colorspace, the Image will be corrected in some cases.
  - When converting a PDF with an Image utilizing an invalidly specified ICC Colorspace, processing has been improved to finish without raising an error in some cases.
- SF47321 – Corrects an issue in the Flattener would create Outlines for Text even if the useTextOutilnes parameter was not set.
- SF47366 – Corrects an issue where the Quad for 90 degree increments of rotation of R2L text would have its values misplaced leading to being unable to determine the proper angle of rotation.
- SF47377 – Corrects an erroneous Out of Memory error that could be raised when rendering a document using the CMYKA colorspace.
- SF47393 – corrects a problem where creating a new Cos Stream from an existing DCT compressed Cos stream could result in endless processing.
  - The decoding of DCT-compressed images on a page has been improved to be more resilient in some cases, images that previously caused errors to be raised can now be processed
- SF47400 – Corrects a problem rendering a Page where many elements of Text using the Clipping Path Rendering Mode could cause some to disappear.
- SF47402 – Address a problem Flattening a XFA document where a \<dateTimeEdit> field could have the improper formatting when flattened using Forms Extension.
- SF47403 – Corrects an error that could occur using PDFOptimizer on some documents with PDF Annotation Appearances.

## APDFL v18.0.5PlusP1y (January 16, 2026)
...
## APDFL v18.0.5PlusP1x (December 11, 2025)

New Features:

- Add PDOCRRecognizePage C++ API to OCR plugin to run optical character recognition (OCR) on a page. The previous APDFL release (v18.0.5PlusP1w) introduced OCREngine support in the C++ interface, along with the OCRImage sample that demonstrates importing a bitmap into a PDF and adding OCR text. This release builds upon that functionality by adding the PDOCRRecognizePage API, which rasterizes an entire page and performs OCR on the resulting image. A new OCRPage sample is included to demonstrate this workflow.

Changes:

- Update libpng to version 1.6.51 to mitigate vulnerabilities.

Problem Corrections:

- SF47046 – Fixes a problem in PDFOptimizer where an empty image with a Matte SoftMask would downsample incorrectly during optimization.
- SF47214 - Corrects a crash that could occur merging a document with a malformed CFF font.
- SF47256 – Improves Embedding of unembedded Type 0 fonts when possible.
- SF47258 – Corrects a problem with Color Conversion for Inline images in some cases.
- SF47286 – New member has been added to the PDFProcessorPDFAConvertParamsRec structure to support conversion to a custom Output Intent.
- SF47291 – Corrects a problem where PDF/A conversion of a document with Type 3 fonts could result in empty page content in rare scenarios.
- SF47307 – Fixes an issue where PDFOptimizer could ignore some images in rare situations.
- SF47313 - Enhancement Request – Document PDF/A Compression Behavior in APDFL Reference Manual
- SF47315 - Corrects a problem with 32-bit PNG export using the makeBufferedImage() method of the Image class of the Java interface.
- SF47325 – Corrects a problem where Optimizing a PDF with Stencil Mask images could result in unexpected changes to the image.

## APDFL v18.0.5PlusP1w (November 20, 2025)

New Features:

- **Added support for turning an image into a PDF document with extractable text with C++ using OCR.** A new functionality has been implemented in C++ that converts images into fully searchable PDF documents by integrating OCR technology. This enhancement enables users to generate PDFs with extractable text derived directly from image content.
- **Added a new OCR sample for C++ to demonstrate the new capabilities**
- **New OCREngine plugin that provides OCR APIs**
- **Added C++ OCR APIs:**
  - **Added C++ PDEForm creation API**
    - PDOCRCreateForm - take a PDEImage and create a PDEForm containing extractable text
  - **Added C++ APIs to control OCR options**
  ...
  - **Add APIs for managing the plugin**
...
- SF46887 – The Font lookup done during PDF rendering has been improved to try suitable alternatives when no other font can be found on the OS.
- SF47188 – Performance of Optimizer has been improved for documents with heavy usage of Type 3 Fonts.
- SF47190 – When the in-use OutputIntent profile matches the Target profile of ColorConversion, Images that are already of the desired colorspace will not be converted.
- SF47198 – Corrects a problem during PDF/A conversion where some Images with SMasks could become unexpectedly cropped.
- SF47210 -
  - Corrects an issue where PDF/A Part 2 or later conversion of a document with Widget fields could lead to an inflated result.
  - Corrects an issue where PDF/A-1 conversion could result in an incompliance for TrueType fonts.
- SF47220 – Corrects PDFOptimizer failing to recompress Images with JPEG compression at Maximum quality when not eligible for Downsampling.
- SF47232 – Corrects a problem in the PDFOptimizer where some images could have color changes when compressed.
- SF47238 – Corrects a problem where converting some documents to PDF/A Part 2 or later would result in Font incompliances with regard to the /CIDSet.
- SF47252 – Corrects a crash that could occur acquiring the WordList for some PDF documents.
- SF47254 – Corrects the capitalization of an include statement in a public header file that affected cross compilation.
- SF47260 – Corrects an erroneous error that could occur for PDFOptimizer when the PDFOptimizerDoNotConvertDeviceNImages option is enabled for Images not eligible for Downsampling.
- SF47266 - Font merging at save time has been improved for Type 0 fonts.
- SF47290 – Corrects a problem in the PDFOptimizer where some images could have color changes when compressed.
...
- Added Linux32 to the release platforms.
...
- SF#46819 - Corrects rendering of a document when Transparency is present where the colors would be slightly different than expected.
...
## APDFL v18.0.5PlusP1h (March 10, 2025)
...
## **Datalogics APDFL v18.0.5PlusP1d (November 13, 2024)**
